...
def format_quick_actions(
    ...
    live_operator_now_labels = {"Позвать менеджера", "Подключить менеджера", "Передать администратору"}

    # TSK-02 продолжение (2026-08-29): "позвать живого человека сейчас" — это ~48 мест в
    # policy/__init__.py плюс ещё пара в chat_service.py, под тремя РАЗНЫМИ подписями кнопки
    # ("Позвать менеджера", "Подключить менеджера", "Передать администратору") — ни одно место
    # не знает про часы работы. Вместо правки каждого — единая точка форматирования уже видит
    # company (working_hours_schedule/timezone), поэтому именно здесь ночью подменяем ЛЕЙБЛ (не
    # value: сообщение всё так же уходит как есть, чтобы сработал честный after-hours хук в
    # policy на этапе подтверждения, а не generic contact_prompt). Не обещаем живого менеджера,
