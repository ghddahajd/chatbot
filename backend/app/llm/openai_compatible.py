            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "цены, диагнозы или гарантии, если их не называл сам клиент. Не пиши, "
                        "предоставил ли клиент имя и телефон — это уже отдельно показано в "
                        "карточке заявки, и то, что клиент НАПИСАЛ что-то похожее на имя, не "
                        "значит, что система его распознала (например 'Тест Тестов' — не "
                        "настоящее имя). Если клиент называл НЕСКОЛЬКО номеров телефона и бот "
                        "НЕ отвечал на них, что номер неполный/некорректный — в карточке "
                        "показан только один, поэтому прямо процитируй все такие номера в "
                        "саммари, чтобы оператору не пришлось искать их в переписке. Но если "
                        "бот сам ответил на какой-то номер, что он неполный/некорректный — "
                        "это не валидный контакт, не указывай его вообще, упомяни только "
                        "номер(а), которые бот принял. Ответь только текстом саммари, без "
                        "вступлений и списков."
                    ),
                },
