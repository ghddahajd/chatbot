...
MEDICAL_KEYWORDS = {
    ...
    # отдельно "распух"/"дышать"/"дыхание" (та же живая находка 2026-08-19, что уже была
    # добавлена в HARD_RESTRICTED_KEYWORDS в __init__.py, но не в этот, отдельный список —
    # is_restricted_question() в restricted.py читает именно MEDICAL_KEYWORDS отсюда, так что
    # реальный гейт №1 всё это время оставался дырявым несмотря на утренний фикс). Оба списка
    # держим синхронно, иначе будем находить один и тот же класс бага заново.
    "горит",
    "слабость",
    "распух",
    "дышать",
