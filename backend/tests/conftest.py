...
def _prepare_clients_dir(temp_dir: Path) -> Path:
    ...
    template_dir = BACKEND_DIR / "data" / "client_template" / "medical_sample"
...
def managed_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, Path]:
    ...
    monkeypatch.setenv("WATCHDOG_STATE_FILE", str(tmp_path / "watchdog_state.json"))
    monkeypatch.setenv("SYSTEM_EVENTS_FILE", str(tmp_path / "system_events.jsonl"))
