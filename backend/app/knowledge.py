...
DEFAULT_PHRASEBOOK = {
    ...
    # rosh_import_demo уже задаёт своё ("медицинский центр") — дефолт нужен только для
    # клиентов, которые этот ключ не переопределяют (иначе company_overview падает на пустую
    # строку). До company_overview (2026-08-28) этот ключ нигде в коде не читался.
    "company_type": "компания",
    "service_word": "услуга",
    ...
    "price_disclaimer": (
        "Это предварительная стоимость. Точную схему и цену определит врач на консультации — "
        "без неё процедуру не проводим."
    ),
    "booking_bridge": "Записаться тоже можно — оставьте телефон и удобное время, менеджер подтвердит.",
    "unknown_service": [
    ...
    "off_topic": [
        "Такое подскажу не я — здесь помогу с услугами, ценами и записью. Что из этого интересует?",
        "Тут я не сориентирую, а вот по услугам, ценам, записи и контактам — пожалуйста. Что подсказать?",
        "Это не по моей части. Могу показать услуги центра или подключить менеджера.",
    ],
    "off_topic_body_redirect": [
        "Это не совсем наш профиль напрямую, но если хотите — можем предложить консультацию, там подскажут точнее.",
        "Такое мы обычно не ведём сами, но могу передать вопрос менеджеру или предложить консультацию специалиста.",
        "Это скорее не к нам, но если удобно — предложу консультацию, чтобы не гадать заочно.",
    ],
    "off_topic_body_redirect_repeat": "Как и раньше — это не совсем наш профиль. Если понадобится помощь, я на связи.",
    "lab_tests_available": [
        "Да, сдать анализы у нас можно. Точный список и как записаться подскажет менеджер.",
        "Да, такая возможность есть. Уточните у менеджера, какие анализы нужны и как записаться.",
    ],
    "contact_prompt": "Оставьте имя и телефон, и менеджер сможет связаться с вами позже.",
    "booking_when_prompt": "Когда вам удобно?",
    "booking_when_today": "Проверим возможность записи сегодня. Оставьте номер телефона — менеджер свяжется с вами и подтвердит время.",
    # и до открытия, и после закрытия: «уже закрыты» в 8 утра было бы неправдой
    "booking_when_today_closed": (
        "Сейчас мы не работаем. Оставьте номер телефона — менеджер свяжется с вами, как только откроемся, "
        "и подберёт ближайшее время."
    ),
    "booking_when_tomorrow": "Хорошо, подберём время на завтра. Оставьте номер телефона — менеджер предложит варианты.",
    "booking_when_later": "Хорошо, подберём удобное время в ближайшие дни. Оставьте номер телефона — менеджер предложит варианты.",
    "booking_phone_prompt": "Оставьте, пожалуйста, номер телефона — менеджер свяжется с вами и подтвердит время.",
    # телефон клиники дописывается в коде, если он есть
    "booking_change_prompt": (
        "Отменить или перенести запись поможет менеджер. Оставьте, пожалуйста, номер телефона, на который "
        "вы записаны, — он найдёт вашу запись и свяжется с вами."
    ),
    "booking_change_success": "Спасибо. Передали менеджеру — он найдёт вашу запись и свяжется с вами.",
    "booking_contact_prompt": (
        "Чтобы оставить заявку, напишите имя, телефон и удобное время. Напомню: перед "
        "процедурой всегда нужна консультация врача — с неё и начнём, а менеджер подтвердит детали."
    ),
    # Та же просьба, но когда бронируемая услуга — САМА консультация (is_consultation_only_service):
    # напоминание "перед процедурой нужна консультация" тут звучало бы как масло масляное.
    "booking_contact_prompt_no_consultation": (
        "Чтобы оставить заявку, напишите имя, телефон и удобное время. Мы передадим заявку, "
        "а менеджер подтвердит детали."
    ),
    # Живой баг (аудит §2026-08-22, "Ниже"): "он увидит историю" не даёт обязательства
    # связаться — слабее, чем complaint_escalation ("свяжется с вами в ближайшее время") для
    # по сути того же хэндоффа. Выравниваем формулировку.
    "handoff_message": "Передаю диалог менеджеру — он свяжется с вами в ближайшее время. Можете дописать детали, он увидит историю.",
    # 2026-08-28, требование клиента (TSK-02): ночью/в закрытые часы честно предупреждаем,
    # что оператор недоступен, вместо "свяжется в ближайшее время" — и сразу просим контакт,
    # не оставляя клиента ждать несуществующего ответа. {working_hours} — тот же текст, что и
    # везде, подтягивается автоматически, если часы работы поменяются.
    # нет связи с Telegram, карточка оператору не ушла; телефон клиники дописывается в коде, если он есть
    "operator_unreachable": (
        "Не получилось сразу связаться с менеджером. Оставьте, пожалуйста, номер телефона — "
        "перезвоним, как только сможем."
    ),
    "operator_after_hours": (
        "Оператор сейчас недоступен — нерабочее время (часы работы: {working_hours}). "
        "Оставьте, пожалуйста, имя и телефон — свяжемся с вами в начале рабочего дня."
    ),
    # §5 скрипта: жалоба на сервис/врача/результат, возврат денег/договор/юридика, угроза
    # отзывом — сразу передаём живому администратору, без попыток бота самому урегулировать.
    "complaint_escalation": (
        "Это важный вопрос, и по нему лучше сразу поговорить с администратором клиники — "
        "передаю ваш диалог коллеге, он свяжется с вами в ближайшее время."
    ),
    # Живой баг (демо-тестирование, 2026-08-24): раньше такие сообщения получали шаблон
    # regulated_advice ("подключить менеджера?") — без какой-либо реальной помощи. Тон —
    # тёплый, не роботический; номер горячей линии клиент подтвердил сам (8-800-2000-122,
    # телефон доверия). Обязательно передаём оператору (TRANSFER_OPERATOR) — не только
    # даём номер, живой человек тоже должен увидеть сообщение.
    "self_harm_crisis": (
        "Мне важно, что вы сейчас чувствуете, и я хочу, чтобы рядом был кто-то, кто может "
        "по-настоящему помочь. Пожалуйста, позвоните на бесплатный круглосуточный Телефон "
        "доверия: 8-800-2000-122 — там ответят прямо сейчас. Я также передаю ваше сообщение "
        "администратору клиники."
    ),
    # Живой баг (аудит §2026-08-22): пока сессия WAITING_OPERATOR, бот отвечал буквально
    # пустой строкой на ЛЮБОЕ сообщение, кроме контактных данных — включая жалобы/угрозы
    # отзывом. complaint_escalation текст тут не годится дословно (он про САМ момент передачи
    # "передаю диалог коллеге" — а тут человек уже передан и ждёт), нужен отдельный текст
    # именно про "уже в очереди, не молчим".
    "waiting_operator_complaint_ack": (
        "Понимаю, ситуация неприятная. Ваше сообщение видит администратор вместе со всей "
        "историей переписки — он ответит вам в ближайшее время."
    ),
    ...
    # 5+ минут, клиент раньше просто застревал на waiting_operator_ack без выхода. Оффер —
    # ровно один раз за сессию (см. Session.operator_return_offered), чтобы не пинг-понговать.
    ...
    "bot_identity_confirm": [
        # Живой баг (ручное тестирование пользователем, 2026-08-26): фраза одна на ЛЮБОЙ
        # вопрос про личность бота — и на да/нет-вопрос ("ты бот?"), и на открытый ("ты
        # кто?"). "Да," в начале звучит нормально для первого, но нелепо для второго ("ты
        # кто? — Да, ..."). Без "Да," фраза работает одинаково хорошо в обоих случаях.
        "С вами общается AI-ассистент компании. Подскажу по услугам, ценам и записи — если нужен живой человек, могу подключить менеджера.",
        "Отвечает AI-ассистент, не живой человек. Могу помочь с услугами, ценами и записью, а менеджера подключу по запросу.",
    ],
    "operator_soft_offer": "Могу попробовать помочь здесь — опишите, что вас интересует. Или сразу соединю с менеджером.",
    # Живой баг (ручное тестирование пользователем, 2026-08-26): после operator_soft_offer
    # ответ вроде "сначала спрошу тут" (без конкретного вопроса, просто "продолжаю в чате")
    # раньше падал в общий off_topic-шаблон ("Тут я не сориентирую...") — нелепо звучит как
    # ответ на несуществующий "вопрос", раз пользователь ничего конкретного ещё не спросил.
    "operator_offer_declined_continue": "Хорошо, слушаю — что вас интересует?",
    # Клиент недоволен/уходит (FRUSTRATION_LEAVING_KEYWORDS) — не "слушаю", а извинение и контакт.
    # Вопрос про цикл/зачатие, на который в базе нет ответа (REPRODUCTIVE_HEALTH_KEYWORDS) — вместо
    # пустого "услуга, цена или запись?". Без цены/названия услуги: клиент-специфика ниже —
    # кнопка консультации показывает услугу, цену уточняют отдельно.
    "health_question_referral": (
        "Такие вопросы лучше обсуждать с врачом — по переписке точно не подскажу. "
        "Могу записать вас на консультацию или передать вопрос менеджеру."
    ),
    "frustration_recovery": (
        "Извините, что не получилось помочь сразу. Оставьте, пожалуйста, телефон — менеджер "
        "свяжется с вами и ответит на ваш вопрос лично."
    ),
    # Живой баг (аудит §2026-08-22, F-13): "возможно есть удалённый формат консультации для
    # вашего случая" — ничем не подтверждённое обещание (клиника только очно, ни в услугах,
    # ни в конфиге клиента нет ни одного упоминания дистанционного/онлайн-приёма), которое
    # к тому же было захардкожено одной строкой без вариативности — 3 разных сообщения в
    # одной сессии получали дословно один и тот же текст. {city} — подставляется город клиники.
    "location_mismatch_offer": [
        "Очный приём только в {city}. Уточните, пожалуйста, у менеджера, какой формат записи подойдёт в вашем случае.",
        "Приём в {city} проходит только очно. Могу подключить менеджера — он подскажет, что можно сделать в вашей ситуации.",
        "У нас очный приём, клиника в {city}. Подскажите менеджеру ваш случай — он сориентирует по формату записи.",
    ],
    "regulated_soft_offer": [
        (
            "Это важный вопрос, и его лучше обсудить со специалистом, не по переписке. "
            "Подключить сейчас, или если срочно — звоните нам напрямую или в скорую (103)?"
        ),
        (
            "Понимаю, здесь нужен специалист, а не мои догадки по переписке. "
            "Оставить контакт, подключить менеджера сейчас, или при срочности звоните нам напрямую либо в скорую (103)?"
        ),
        (
            "Такое лучше оценит специалист очно. "
            "Передать вопрос менеджеру или помочь оставить контакт? Если срочно — звоните нам напрямую или в скорую (103)."
        ),
    ],
    # Тот же soft-offer, но для бытовых вопросов ("а больно?", "это нормально?") без реально
    # острых сигналов — см. escalation_urgency в policy/_medical_referral_result. Без "скорая
    # (103)": этот номер уместен для действительно острых ситуаций, не для вопроса про укол.
    "regulated_soft_offer_calm": [
        (
            "Заочно тут лучше не гадать — точнее подскажет специалист. "
            "Подключить менеджера сейчас, или оставите контакт?"
        ),
        (
            "Такие детали лучше уточнить у специалиста, а не по переписке. "
            "Могу подключить менеджера сейчас или взять ваш контакт."
        ),
        (
            "Это стоит обсудить со специалистом напрямую. "
            "Передать вопрос менеджеру или помочь оставить контакт?"
        ),
    ],
    # Живой баг (ручное тестирование, 2026-08-29): те же 2 варианта выше, но для нерабочих
    ...
    "regulated_soft_offer_after_hours_urgent": (
        "Это важный вопрос, и его лучше обсудить со специалистом. Сейчас нерабочее время "
        "(часы работы: {working_hours}) — если срочно, звоните нам напрямую или в скорую "
        "(103); если может подождать, оставьте, пожалуйста, контакт — свяжемся в начале "
        "рабочего дня."
    ),
    "regulated_soft_offer_after_hours": (
        "Заочно тут лучше не гадать — точнее подскажет специалист. Сейчас нерабочее время "
        "(часы работы: {working_hours}) — оставьте, пожалуйста, контакт, и с вами свяжутся "
        "в начале рабочего дня."
    ),
    # §3.2 скрипта: на первое сообщение с описанием симптома (не названием конкретной услуги)
    # сначала короткий уточняющий вопрос, а не сразу предложение услуги — bot не выбирает
    # процедуру за врача с первой реплики. См. policy/__init__.py::_curated_match_is_explicit_service_mention.
    "symptom_followup_question": [
        "Понимаю, это неприятно. Расскажите, пожалуйста, давно ли это началось?",
        "Спасибо, что рассказали. Подскажите — это продолжается уже давно, или началось недавно?",
        "Понимаю. Уточните, пожалуйста — вы уже проходили осмотр по этому поводу или обращаетесь впервые?",
    ],
    # §3.4 скрипта: "не понимаю, к кому мне лучше попасть" — не называем конкретную
    # специализацию врача (в данных клиники специальность заполнена только у гинеколога),
    # общая формулировка "специалист/профильный врач" остаётся честной для остальных докторов.
    "doctor_uncertain": [
        (
            "Это нормально — для этого и есть первичная консультация: специалист проведёт "
            "осмотр и, если нужно, направит к профильному врачу клиники. Начнём с записи "
            "на консультацию?"
        ),
        (
            "Такое бывает — начнём с первичной консультации, там определят, какой специалист "
            "нужен именно вам. Записать?"
        ),
        (
            "Понимаю, сориентироваться непросто. Первичная консультация как раз для этого — "
            "специалист осмотрит и направит дальше, если нужно. Хотите записаться?"
        ),
    ],
    "engagement_offer_1": (
    ...
    "clarify": "Скажите, пожалуйста, что вас интересует — услуга, цена или запись? Так отвечу точнее.",
    ...
    "contact_cancelled": "Хорошо, контакт пока не берём. Могу подсказать по услугам, ценам или позвать менеджера, если понадоблюсь.",
    "booking_cancelled": "Хорошо, заявку пока не передаю. Могу подсказать по услугам, ценам или позвать менеджера, если понадоблюсь.",
    "general_cancelled": "Хорошо, ничего пока не делаем. Могу подсказать по услугам, ценам или позвать менеджера, если понадоблюсь.",
    "lead_success": "Спасибо. Передали ваши контакты менеджеру. С вами свяжутся для уточнения деталей.",
    "booking_success": (
        "Спасибо. Заявку передали. Напомню: перед процедурой нужна консультация врача — "
        "менеджер свяжется, чтобы согласовать время и детали."
    ),
    # Пара к booking_success — когда бронируемая услуга сама и есть консультация
    # (is_consultation_only_service), напоминание про консультацию перед процедурой лишнее.
    ...
    # Живой баг (2026-08-28): "расскажи о клинике" уходило в off_topic — у фиксированного
    # списка категорий классификатора нет варианта "общий вопрос о компании". Ответ строго
    # из уже проверенных структурных полей (имя/тип/город/часы работы), без выдумывания
    # содержания — тот же принцип, что и у остальных clinic_info-ответов.
    "company_overview": (
        "{company_name} — {company_type} в {city}. Часы работы: {working_hours}. "
        "Подскажу по услугам, ценам и записи — что вас интересует?"
    ),
    ...
    "objection_price": (
        "Понимаю, вопрос цены важен. В стоимость обычно входит осмотр специалиста и подбор "
        "варианта именно под вашу ситуацию, а не типовая схема. Рассказать подробнее, что входит, "
        "чтобы было понятно, за что вы платите?"
    ),
    "objection_hesitation": (
        "Конечно, торопиться не будем. Подскажите — смущает цена, время или хочется больше узнать "
        "о процедуре? Так я сразу отвечу на главный вопрос."
    ),
    # §3.5 скрипта: "думаю"/"посоветуюсь" именно НА ЭТАПЕ ЗАПИСИ (pending_action ==
    # BOOKING_CONTACT) — не тот же вопрос "что смущает" (objection_hesitation), а мягкая
    # альтернатива жёсткому закрытию: придержать интерес без обязательств, не настаивать.
    "booking_hesitation_hold": [
        (
            "Конечно, торопиться некуда. Могу пока зафиксировать ваш интерес без "
            "обязательств — если не подойдёт, просто отмените. Оставить контакт предварительно?"
        ),
        (
            "Понимаю, ничего страшного. Могу просто отметить, что вы интересовались, без "
            "каких-либо обязательств с вашей стороны — оставить контакт на этот случай?"
        ),
    ],
    "objection_competitor": (
        "Возможно, так и есть — компании отличаются оснащением и опытом специалистов. У нас "
        "процедуру проводит специалист, который ведёт пациента от осмотра до результата. Решение, "
        "конечно, за вами — рассказать подробнее про специалиста и процесс, чтобы было проще сравнить?"
    ),
    "objection_guarantee": (
        "Точный прогноз даёт специалист после осмотра — это честнее любых обещаний заранее, ведь "
        "каждый случай индивидуален. Специалист подберёт вариант именно под вашу ситуацию и будет "
        "сопровождать на всех этапах. Записать на консультацию, где подробно расскажут об ожидаемом результате?"
    ),
    "objection_pain_fear": (
        "Это частое беспокойство, и его стоит обсудить именно со специалистом — он расскажет про "
        "обезболивание и возможные реакции конкретно для вашей процедуры. Хотите, запишу вас на "
        "консультацию, где как раз можно всё это обсудить перед тем, как принимать решение о самой процедуре?"
    ),
    "objection_backoff": (
        "Понимаю. Не буду торопить — пришлю информацию, а вы напишите, когда будет удобно "
        "вернуться к вопросу."
    ),
...
def is_consultation_only_service(service: Optional[Service]) -> bool:
    """true, если сама услуга — это консультация (а не процедура с консультацией перед ней).

    Нужно, чтобы не напоминать "перед процедурой нужна консультация врача" в ответ на
    запись именно на консультацию — иначе получается масло масляное.

    2026-08-29: было точное сравнение category == "консультации" — работает для
    rosh_import_demo (категория называется буквально так), но молча сломалось бы для
    будущего клиента с другой формулировкой ("Консультации врача", "Первичная
    консультация"). Ключ "консультац" по category ИЛИ по name ловит любую форму —
    поведение для текущего клиента не меняется (проверено: в его данных ровно одна
    услуга содержит этот корень, и это она же)."""

    if service is None:
        return False
    return "консультац" in normalize_text(service.category) or "консультац" in normalize_text(service.name)
