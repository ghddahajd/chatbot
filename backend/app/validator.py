...
def _validate_fact_constraints(answer: str, context: dict[str, Any]) -> bool:
    ...
    # Живой баг (аудит §2026-08-22, "Ниже"): "чем вы занимаетесь" — модель сама (mode:
    # safe_complete, не шаблон) генерировала один абзац из 24-28 услуг через запятую вместо
    # "1-3 предложения, без простыней" (раздел 1 скрипта). Сопоставлять названия услуг из
    # контекста с текстом ответа ненадёжно — в живом предложении они стоят в разных падежах
    # ("биоревитализацию", не "Биоревитализация" как в каталоге), а многословные названия
    # делят общие слова ("лазерная терапия"/"лазерная шлифовка"/"лазерный пилинг" —
    # совпадение по одному "лазер*" задвоило бы счёт). Считаем запятые в самом ответе.
    #
    # Регрессия (пойман пользователем сразу после первого фикса): "покажи услуги"/"хочу
    # услуги" — тот же question_type, но это ЯВНАЯ просьба показать список, а не расплывчатый
    # small-talk-триггер вроде "чем вы занимаетесь". Полный список тут — правильный ответ по
    # существу, не "простыня". Пропускаем лимит, если сообщение само явно просит список
    # (SERVICE_LIST_FAST_MESSAGES/SERVICE_LIST_KEYWORDS — тот же набор, которым локальный
    # классификатор сам распознаёт этот интент, см. policy/intent.py).
    if question_type == "list_services" and answer.count(",") > MAX_COMMAS_IN_LIST_ANSWER:
        user_message = normalize_text(str(context.get("user_message") or ""))
        explicitly_requested = user_message in SERVICE_LIST_FAST_MESSAGES or contains_keyword(
            user_message, SERVICE_LIST_KEYWORDS
        )
